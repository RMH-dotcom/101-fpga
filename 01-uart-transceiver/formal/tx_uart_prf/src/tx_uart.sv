...
// The full picture:
// - Block 3 counts 1084→0, fires baud_stb each time it hits 0
...
module tx_uart (
  ...
  // My FPGA ticks 125 million times per second. A UART receiver only expects 115200
  // bits per second. This number (1085) is how many clock ticks must pass before I
  // send the next bit.
  parameter    CLOCKS_PER_BAUD = 1085; // 125MHz / 115200 baud
...
// else if (state != IDLE)  -> reload counter
  always_ff @(posedge i_clk)
    if (i_wr && !o_busy) begin
      counter <= CLOCKS_PER_BAUD - 1'b1; // Begin counting down from 1084
    ...
    else if (is_state != IDLE) begin// If baud tick fired in mid-transition,
      counter <= CLOCKS_PER_BAUD - 1'b1; // Reload the counter
      baud_stb <= 1'b0; // And clear the baud_stb!
    end
